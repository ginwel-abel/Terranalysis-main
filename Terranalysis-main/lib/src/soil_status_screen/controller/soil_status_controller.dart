...
class SoilStatusController extends GetxController {
  ...
  RxString deviceIotStatus = 'OFF'.obs;
  RxString deviceIsReading = 'NO'.obs;

  void fetchData() {
    try {
      database.child('').onValue.listen((DatabaseEvent event) {
        ...
        if (dataSnapshot.value != null) {
          ...
          SoilQuality soilQuality = SoilQuality(
            ...
            moisture: double.parse(data['Moisture'].toString()),
            soilPh: double.parse(data['SoilPH'].toString()),
            phosphorus: double.parse(data['Phosphorus'].toString()),
            nitrogen: double.parse(data['Nitrogen'].toString()),
            potassium: double.parse(data['Potassium'].toString()),
          );
          ...
          deviceIotStatus.value = data['Status'].toString();
          deviceIsReading.value = data['isDone'].toString();
          // NITROGEN
          if (nitrogen.value < 240) {
            nitrogenStatus.value = "Low";
          } else if (nitrogen.value >= 240 && nitrogen.value < 480) {
            nitrogenStatus.value = "Medium";
          ...
          if (soilPH.value >= 4.5 && soilPH.value < 6.5) {
            ...
            if (soilPH.value >= 7.6 && soilPH.value < 8.5) {
              soilPHStatus.value = "Low";
          ...
          nitrogenPercent.value = nitrogen.value > 200 ? 1 : (nitrogen.value / 200);
          phosphorusPercent.value = phosphorus.value > 100 ? 1 : (phosphorus.value / 100);
          potassiumPercent.value = potassium.value > 400 ? 1 : (potassium.value / 400);
          soilPHpercent.value = soilPH.value > 10 ? 1 : (soilPH.value / 10);
        }
        calculateRecommendation();
      });
    } on Exception catch (e) {
      log("ERROR (fetchData) $e");
    }
  }
